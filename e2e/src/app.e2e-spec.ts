...
describe("first-app-lesson-10 app", () => {
  ...
  it("should have a housing-location component", async () => {
    expect(
      await element.all(by.css(".listing > .listing-location")).get(0).getText()
    ).toEqual("Chicago, IL")
  })

  it("should have a title in the housing-location component", async () => {
    expect(await element.all(by.css(".listing > h2")).get(0).getText()).toEqual(
  ...
  it("should have a details page at /details/9", async () => {
    ...
    expect(
      await element.all(by.css("app-details > p")).get(0).getText()
    ).toEqual("details works!")
  })
